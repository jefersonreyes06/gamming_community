...
class StreamBuilderState extends ConsumerState<StreamBuilderJoined> {
  ...
  Widget build(BuildContext context) {
    ...
    user.when(
      data: (user) {
        final List<String> communityIds = user!.communityIds ?? []; // Obtain communityIds from user data
        ...
        return joinedCommunities.when(
            ...
            error: (e, _) => Center(child: Text('Error to load communities: ${e.toString()}')),
            data: (communities) =>
                ListView.builder(
      ...
      error: (e, _) => Center(child: Text('Error to load user: ${e.toString()}')),
      loading: () => const Center(
        child: CircularProgressIndicator(),
      ),
    )
