...
class ProfilePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final auth = ref.watch(
        authRepositoryProvider); // Functions such as Log out account provider by Auth
    final myUid = ref.watch(
        authStateProvider.select((state) => state.value!.id));
    final user = ref
        .watch(userProvider(uid ?? myUid))
        .value; // Obtain user data... path: users/{userId}
                ...
                children: [
                  ...
                  Column(
                      children: [
                        ...
                        Row(
                          children: [
                            CustomIconStorage(consult: storage.getImageUrl("images/$uid/${user.profilePath}"), radius: 36),
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(user.name ?? "User", style: TextStyle(
                                      fontSize: 30,
